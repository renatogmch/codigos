...
# As bases de dados dos trabalhadores sao enormes e estao divididas em arquivos individuais entre 
# regioes (em 2006) ou estados (2008 adiante). Por motivos de RAM, cada ano requer uma funcao individual
# para carregamento e manipulacao, sendo o resultado de cada iteracao em loop "foreach" salvo 
# arquivo em individual de nome trab[ANO].rds
...
# Manipulacao ----
# Summarise pra CNPJ, criacao de variaveis
        ...
        .packages = c('here','tidyverse')
          ...
          files %>%
            ...
            summarise(tam_estab = min(tam_estab),
                      ...
                      # vinculos eh a contagem de linhas (cpfs) para cada cnpj
                      vinculos = n(),
                      # remuneracao media eh a soma das remuneracoes anuais medias de cada funcionario 
                      # dividido pelo num de funcionarios
                      rem_med = sum(rem_med)/n(),
                      # pro_superior eh a proporcao de funcionarios com ensino superior do total
                      prop_superior = length(cpf[grau_instr>7]) / length(cpf),
